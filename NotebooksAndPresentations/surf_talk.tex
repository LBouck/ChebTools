...
\noindent
\begin{minipage}{.4\linewidth}
\begin{block}{1-D Rootfinding}
\begin{itemize}
\item We now have the possible $x$ values for where there are common roots
\item We employ a well known 1-D rootfinding method again using Chebyshev Polynomials
\end{itemize}
\end{block}
\end{minipage}%
\hspace{.01\linewidth}
...
\begin{tabular}{ |c|c|c|c|c|  }
 ...
 \hline
 Functions							& Error 	& Time (s)	& Error	& Time (s)\\
 \hline
...
\end{tabular}
\begin{align*}
F_1(x,y) &= T_3(x)-13T_1(x)\\
 F_2(x,y) &= T_3(y)-13T_1(y)\\
G_1(x,y) &= \cos(\pi x)(y-2)\\
 G_2(x,y) &= (y-.9)(x-2)\\
H_1(x,y) &= \cos\left(\pi x-\frac{\pi}{10}\right)(y-2)\\
H_2(x,y) &= (y-.1)(y-.9)(x-2)
\end{align*}
}
\frame{
...
\item The Summer Undergraduate Research Fellowship program
\end{itemize}
...
$[1]$ G. Golub and C. Van Loan. {\it Matrix Computations}. The Johns Hopkins University Press, Baltimore, 2013.\\
 $[2]$ A. Townsend. {\it Computing with Functions in Two Dimensions} (Doctoral Dissertation). Oxford University, 2014.\\
 $[3]$ L. N. Trefethen, {\it Approximation Theory and Approximation Practice}, SIAM, Philadelphia, 2013.\\
}
