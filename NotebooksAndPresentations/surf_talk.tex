...
\begin{itemize}
\item This is a global root finding problem: we want all solutions inside $\Omega$
\item Root finding is has applications in many fields
\item An application for NIST would be in making flash calculations to determine properties of a fluid from its equation of state
\end{itemize}
}
%\frame{
%\frametitle{Example}
...
\frametitle{Example}
$f(x,y)=x^2-y^2$ and $g(x,y)=\sin\left(\pi\sqrt{x^2+y^2}\right)$ with the square domain $\Omega=[-1,1]^2$
%We are finding where $f$ and $g$'s zero level sets intersect inside $\Omega$
\begin{center}
\begin{figure}
...
\frame{
\frametitle{Local Methods May Not Be Good for Global Root Finding}
...
\caption{Note that initial guesses in white areas converged to roots outside our domain and the basin of attraction for $(0,0)$ is relatively small}
\end{figure}
\end{center}
}
...
\bigskip
\begin{block}{Goal:}
To develop a truly global root finding method
\end{block}
}
...
\begin{figure}
\includegraphics[width=\linewidth]{cheb_poly_plot.pdf}
\end{figure}
\end{minipage}%}
}
...
\frame{
\frametitle{Chebyshev polynomials are accurate and numerically stable}
\begin{itemize}
\item Typical interpolation is susceptible to the {\bf Runge phenomenon}
...
\begin{center}
\begin{figure}
\includegraphics[width=1\textwidth]{runge_plot.pdf}
\end{figure}
\end{center}
}
...
%\item Employ a 1-D root finding technique to find all the roots along the already found $x$ and $y$ values
%\end{itemize}
%}
...
\begin{center}
\begin{figure}
\includegraphics[width=.85\textwidth]{pointwise_error_surface.pdf}
\caption{Plot of the pointwise errors of our algorithm}
\end{figure}
\end{center}
}
...
%}
\frame{
\frametitle{2nd: B\'{e}zout Matrix Polynomials}
\begin{itemize}
\item We can create B\'{e}zout matrices at each Chebyshev node along the $x$ or $y$ axis
\item Interpolating the B\'{e}zout matrices gives us a matrix polynomial $B(x)$
\end{itemize}

The matrix polynomial can be expressed as
$$B(x) = \sum_{k=0}^MB_{k}T_{k}(x)$$
...
\item Can be solved by linearizing and solving generalized eigenvalue problem of size $MN\times MN$
\item The most computational intensive part of the algorithm
\end{itemize}
}
...
\frame{
\frametitle{3rd: 1-D Root Finding}
\begin{itemize}
\item We now have the possible $x$ (or $y$) values for where there are common roots
\item Employ a 1-D root finding method again using Chebyshev Polynomials
\end{itemize}
We do the following:
\begin{itemize}
\item Find the 1-D Chebyshev polynomials $p_f(y),p_g(y)$ at the specific $x$ values
\item Utilize 1-D Chebyshev rootfinding techniques
%\item Find the roots of $p_f(y)+p_g(y)$ using a companion matrix method
%	\begin{itemize}
%	\item We do this to account for the degenerate case of $p_g(y)$ or $p_f(y)\equiv 0$
%	\item If $p_g(y)=-p_f(y)$ then we find the roots of $p_f(y),p_g(y)$ individually
%	\end{itemize}
\item Polish the roots with Newton's method
\end{itemize}
}
...
%Similarities:
%\begin{itemize}
%\item The B\'{e}zout Resultant method for 2-D root finding and companion matrix methods for 1-D root finding
...
%Differences:
%\begin{itemize}
%\item The goals of chebfun2 are much more general than just root finding, unlike our code
...
Key Contributions:
\begin{itemize}
\item Replicated Alex Townsend's algorithm in chebfun2 with some modifications in C++
...
\item The SURF/SHIP program and it's directors
\end{itemize}
