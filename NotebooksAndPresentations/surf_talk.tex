...
\begin{block}{The Big Picture: ChebTools}
\begin{itemize}
\item ChebTools is a C++ library for working with Chebyshev expansions
\item Inspired by the Matlab library Chebfun
\item Solving this problem will be a significant feature for ChebTools
\item With a higher level Python interface, ChebTools could be useful for a wide range of users
\end{itemize}
\end{block}
}
\frame{
...
\begin{center}
\begin{figure}
\includegraphics[height=.7\textheight]{pointwise_error_surface.pdf}
\end{figure}
\end{center}
}
...
\begin{center}
\begin{figure}
\includegraphics[height=.55\textheight]{bezout_det_plot.pdf}
\end{figure}
\end{center}
}
...
\item {\bf Issue:} Poor conditioning of matrix polynomial problem $\implies$ need for local refinement
...
Key Contributions:
\begin{itemize}
\item Made significant progress in replicating the 2-D rootfinding algorithm in chebfun2 with some modifications in C++
